...
- Latest validation: `npm.cmd run verify` passes with 32 files / 261 tests and production build, `npm.cmd run qa:viewport` passes 29 scenarios with all visible buttons named and at least 44x44px, `npm.cmd run qa:mobile-touch` passes a 38-step real-touch journey with 12 tracked attempts, the first-install offline cache test covers entry plus lazy chunks, and the local ElevenLabs audit passes at 1487/1487 current lines plus 32/32 reading phonemes.
