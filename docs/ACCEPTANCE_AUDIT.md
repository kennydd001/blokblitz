...
- Tests: `npm.cmd run test` passes with 34 test files / 294 tests.
...
- Browser UI/gameplay QA: `npm.cmd run qa:viewport` passes 56 Chrome scenarios covering the return-aware opening, reduced-motion emulation, first-profile creation, the four-child profile cap, the 75-star calm-mode collection, serialized Buddy/hero rewards, personal next-mission and full-treasure endings, Sterrenreis, the personal-mission Speeltuin, the real runner, core reading/math/discovery modes, forced tier-3 curriculum content, bosses, 332px phones, desktop fullscreen, and mobile landscape.
...
- Milestone 7 is current, not inherited: 34 files / 294 tests, typecheck, lint,
  production build, 56 viewport scenarios, deterministic 40-step touch journey, 1489/1489
...
- Latest validation: `npm.cmd run verify` passes with 34 files / 294 tests and production build, `npm.cmd run qa:viewport` passes 56 scenarios with explicit opening-overlap, reduced-motion, first/capped-profile, 75-star collection, serialized portrait/landscape reward dialogs, 332px next-mission ending, 844x390 full-treasure ending, tiered-Memory, and deterministic advanced-content checks plus all visible buttons named and at least 44x44px, `npm.cmd run qa:mobile-touch` passes a fixed-seed 40-step journey that equips Aqua with 12 tracked attempts, the first-install offline cache test covers entry plus lazy chunks, and the local ElevenLabs audit passes at 1558/1558 stored clips, 1489/1489 current lines, and 32/32 reading phonemes.
