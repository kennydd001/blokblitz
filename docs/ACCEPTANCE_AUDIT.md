...
- Tests: `npm.cmd run test` passes with 34 test files / 295 tests.
...
- Browser UI/gameplay QA: `npm.cmd run qa:viewport` passes 58 Chrome scenarios covering the fresh and returning profile-sign opening, reduced-motion emulation, first-profile creation, the four-child profile cap, the 75-star calm-mode collection, serialized Buddy/hero rewards, personal next-mission and full-treasure endings, Sterrenreis, the personal-mission Speeltuin, the real runner, core reading/math/discovery modes, forced tier-3 curriculum content, bosses, 332px phones, desktop fullscreen, and mobile landscape.
...
- Milestone 7 is current, not inherited: 34 files / 295 tests, typecheck, lint,
  production build, 58 viewport scenarios, deterministic 41-step touch journey, 1489/1489
...
- Product-readiness dimensions were re-audited directly: first impression is
  ...
  fully local; returning children must identify the active profile's unique
