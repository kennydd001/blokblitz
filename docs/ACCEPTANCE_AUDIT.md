...
- Tests: `npm.cmd run test` passes with 39 test files / 340 tests.
...
- Browser UI/gameplay QA: `npm.cmd run qa:viewport` passes 67 Chrome scenarios covering the fresh and returning profile-sign opening on phone, landscape, and desktop; a single-message first-journey cinematic; reduced-motion emulation; first-profile creation; the four-child profile cap; the 78-star calm-mode collection; serialized Buddy/hero rewards; personal next-mission and full-treasure endings; Sterrenreis; the personal-mission Speeltuin; real traced handwriting input; the real runner; core reading/math/discovery modes; forced tier-3 curriculum content; bosses; healthy session boundaries; 332px phones; desktop fullscreen; and mobile landscape. Advanced connected blending now additionally proves that a five-sound chain stays on one row.
- Mobile touch QA: `npm.cmd run qa:mobile-touch` passes a fixed-seed phone-like Chrome route using real `Input.dispatchTouchEvent` touches. The 45-step route taps a wrong returning profile sign and remains safely on boot, taps the correct sign, exercises runner controls and swipes, completes audio-gated Count, selects reading, finishes Klankgrot, earns and equips Aqua through the reward modal, logs 12 attempts, advances one node, and proves the adult-gated playtime extension without a ghost tap.
...
- Milestone 6 is exceeded: the original 12 templates still share the attempt
  ...
  and scaffolded.
- Milestone 7 is current, not inherited: 39 test files / 340 tests, typecheck,
  lint, production build, 67 viewport scenarios, deterministic 45-step touch
  journey, 1524/1524 reachable sentence clips, 32/32 reading phonemes, live Sprintsite shell/code/
...
- Product-readiness dimensions were re-audited directly: first impression is
  ...
  explicitly asks for immediate repetition; answer layouts may repeat twice but
  never expose the same single correct position three rounds in succession;
  early math/discovery modes
  ...
  narration when its visible card is dismissed, is pausable, and
  fully local; profile selection waits for its natural welcome line and audio
  construction failures release gated navigation; returning children must
  identify the active profile's unique
  visual sign, while switching remains parent-gated plus hold-confirmed; visible
  controls are named and >=44px; reduced-motion preferences stop looping CSS
  movement; target-aware history compaction bounds long-term storage; four
  parent-managed profiles, profile identity signs, calm-mode best ratings, and collectible hero unlocks stay isolated; every
  reward source surfaces a non-overlapping equip-or-later choice; passive startup defers WebGL; background work
...
- Latest validation: `npm.cmd run verify` passes with 39 test files / 340 tests and production build; `npm.cmd run qa:viewport` passes 67 scenarios with explicit fresh/returning opening checks on phone, landscape, and desktop, real pointer-drawn Schrijfspoor ink, a 332x807 starter writing board, a 78-star collection, tiered Memory, profile-earned 4/28 starter versus advanced `IJ` letters, a rendered Splitbord remediation model, deterministic five-sound Zoemroute/Woordbouw checks, and all visible buttons named and at least 44x44px; `npm.cmd run qa:mobile-touch` passes a fixed-seed 45-step journey that verifies wrong/correct profile signs, equips Aqua with 12 tracked attempts, and completes the adult playtime extension without a ghost tap; the first-install offline cache test covers entry plus lazy chunks; and the local ElevenLabs audit passes at 1595/1595 stored clips, 1524/1524 current lines, and 32/32 reading phonemes.
