...
- Tests: `npm.cmd run test` passes with 34 test files / 290 tests.
...
- Browser UI/gameplay QA: `npm.cmd run qa:viewport` passes 54 Chrome scenarios covering the return-aware opening, reduced-motion emulation, first-profile creation, the four-child profile cap, the 75-star calm-mode collection, serialized Buddy/hero rewards, Sterrenreis, the personal-mission Speeltuin, the real runner, core reading/math/discovery modes, forced tier-3 curriculum content, bosses, 332px phones, desktop fullscreen, and mobile landscape.
- Mobile touch QA: `npm.cmd run qa:mobile-touch` passes a fixed-seed phone-like Chrome route using real `Input.dispatchTouchEvent` touches. Two consecutive 40-step runs exercised runner controls and swipes, completed audio-gated Count, selected the reading category, finished Klankgrot, earned and equipped Aqua through the reward modal, logged 12 attempts, advanced one node, and returned to the Hub.
...
- Milestone 7 is current, not inherited: 34 files / 290 tests, typecheck, lint,
  production build, 54 viewport scenarios, deterministic 40-step touch journey, 1489/1489
...
- Product-readiness dimensions were re-audited directly: first impression is
  ...
  have persistent thematic goals; audio is natural, callback-serialized across
  scene narration and reward dialogs, cancels narration when its visible card is
  dismissed, is pausable, and
...
- Latest validation: `npm.cmd run verify` passes with 34 files / 290 tests and production build, `npm.cmd run qa:viewport` passes 54 scenarios with explicit opening-overlap, reduced-motion, first/capped-profile, 75-star collection, serialized portrait/landscape reward dialogs, tiered-Memory, and deterministic advanced-content checks plus all visible buttons named and at least 44x44px, `npm.cmd run qa:mobile-touch` passes fixed-seed 40-step journeys that equip Aqua with 12 tracked attempts, the first-install offline cache test covers entry plus lazy chunks, and the local ElevenLabs audit passes at 1489/1489 current lines plus 32/32 reading phonemes.
