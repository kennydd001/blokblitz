...
Every child gets three stable, profile-specific daily missions: one math activity, one reading activity, and one discovery activity. The recommendation engine combines curriculum stage, recurring errors, memory decay, and recent completed modes, so it can revisit a weak sound without serving the same game shell again immediately. Recommendation stages follow real journey milestones (Zoemroute, then Tienbrug), so later content such as money, clock reading, skip-counting, and bridging through ten is never recommended to a new child, but remains reachable in free play.
...
0. **De Sterrenreis** (the story mode, the front door — `ReisScene`/`src/data/journey.ts`): the app boots onto one winding road that ties the full curriculum into a single adventure — *help Buddy carry the lost star home*. The current road has 50 stops, gates, bosses, rescued friends, and the final star across six world colour bands. Its first-grade spiral moves from sound awareness to letter-sound mapping, writing, connected blending and word building; number work moves from quantity sense through splitting and number bonds into structures and operations to 20, with clock reading late. Exactly one node glows; finishing it blooms the node into colour, moves the star, and opens the next step. Old position-based node ids are migrated to the new route without moving a returning child backwards. No reading is required because Buddy and the spoken voice carry the story. A `🎒 Vrij spelen` button opens the Speeltuin.
